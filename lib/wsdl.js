...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  var ns = '';
  console.log('ignored', this.options.ignoredNamespaces, this.ignoredNamespaces);
  console.log('is ignored?', this.options.ignoredNamespaces.indexOf(namespace) === -1);
  console.log('prefixNamespace', prefixNamespace);
  console.log('qualified', ((qualified || first) || soapHeader));
  console.log('should ignore', (!isNamespaceIgnored || this.options.ignoredNamespaces.indexOf(namespace) === -1));
  if (prefixNamespace && ((qualified || first) || soapHeader) && (this.options.ignoredNamespaces.indexOf(namespace) === -1 || !isNamespaceIgnored)) {
    // prefix element
    ns = namespace.indexOf(":") === -1 ? namespace + ':' : namespace;
    console.log('NAMESPACE USED:', ns);
    console.log('NAMESPACE USED:', ns);
    console.log('NAMESPACE USED:', ns);
    console.log('NAMESPACE USED:', ns);
    console.log('NAMESPACE USED:', ns);
